...
struct HTFileJobTests {
  ...
  @Test func testFullJFK() async throws {
    ...
    let task = try await job.start(modelPath: modelPath)
  ...
  @Test func testStop() async throws {
    ...
    _ = try await job.start(modelPath: modelPath)
    try await job.stop()
  ...
  @Test func testParallel() async throws {
    ...
    let jobATask = try await jobA.start(modelPath: modelPath)
    let jobBTask = try await jobB.start(modelPath: modelPath)
